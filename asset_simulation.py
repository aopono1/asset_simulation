...
def main():
    print("===== Asset Projection Simulator =====")
    
    # ユーザー入力
    start_year = int(input("Starting year: "))
    start_age = int(input("Your age at starting year: "))
    initial_assets = float(input("Current investment assets (JPY): "))
    annual_return = float(input("Annual investment return rate (%): ")) / 100
    monthly_investment = float(input("Monthly investment amount (JPY): "))
    end_investment_year = int(input("Year to end monthly investment: "))
    start_withdrawal_year = int(input("Year to start withdrawals: "))
    withdrawal_rate = float(input("Annual withdrawal rate (%): ")) / 100
    ...
    # 英語のカラム名に変更
    display_results.columns = ['Year', 'Age', 'Total Assets (JPY)', 'Monthly Withdrawal (JPY)']
    
    # 5年ごとのデータを表示
    step = 5
    filtered_results = display_results.iloc[::step].copy()
    ...
    print("\nSimulation results saved to CSV file: asset_projection_results.csv")
